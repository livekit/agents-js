...
describe('trace schema rules', () => {
  ...
  it('checks the per-turn invariants', () => {
    ...
    expect(
      checkTrace(spans).some((v) => v.includes('lk.generation_count=3 but 2 generation events')),
    ).toBe(true);
    ...
    // a discarded preemptive attempt's generation sits on the span and counts: the count is
    // the turn's, not the finishing speech's
    spans = soundTrace();
    spans[TURN]!.events = [
    ...
    spans[TURN]!.attributes['lk.generation_count'] = 3;
    expect(checkTrace(spans)).toEqual([]);
    spans[TURN]!.attributes['lk.generation_count'] = 2;
    expect(
      checkTrace(spans).some((v) => v.includes('lk.generation_count=2 but 3 generation events')),
    ).toBe(true);
    // but a generation after the handoff must be the finishing speech's own
    spans[TURN]!.attributes['lk.generation_count'] = 3;
    spans[TURN]!.events[3] = {
      name: 'generation',
      attributes: { 'lk.generation_id': 'speech_9_1' },
    };
    expect(
      checkTrace(spans).some((v) =>
        v.includes('generation speech_9_1 after the last handoff is not its own'),
      ),
    ).toBe(true);

    // a generation the framework emitted always carries its id: one without is malformed,
    // a discarded attempt's before the handoff as much as the finishing speech's after it
    spans = soundTrace();
    spans[TURN]!.events[1] = { name: 'generation', attributes: {} };
    expect(checkTrace(spans)).toContain('agent_turn speech_1: generation without lk.generation_id');
    spans = soundTrace();
    spans[TURN]!.events = [
    ...
    // nothing exported is not a sound trace
    expect(checkTrace([]).some((v) => v.includes('0 traces'))).toBe(true);
