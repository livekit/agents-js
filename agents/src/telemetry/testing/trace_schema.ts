...
export function checkTrace(
  ...
  for (const span of spans) {
    ...
    // the count is of every generation on the turn, a discarded preemptive attempt's included
    const generations = span.events.filter((event) => event.name === 'generation').length;
    const count = span.attributes['lk.generation_count'];
    const countOk =
      (typeof count === 'number' || typeof count === 'string') &&
      Number.isInteger(Number(count)) &&
      Number(count) === generations;
    if (!countOk) {
      violations.push(
    ...
    // generations before the last handoff belonged to the discarded attempts; the ones after
    // it are the finishing speech's own (a realtime tool reply continues the tool call's id)
    const lastHandoff = span.events
      .map((event, index) => (event.name === 'preemptive_generation_discarded' ? index : -1))
      .reduce((a, b) => Math.max(a, b), -1);
    for (const [index, event] of span.events.entries()) {
      if (event.name !== 'generation') continue;
      const id = event.attributes['lk.generation_id'];
      // every generation the framework emitted carries its id, a discarded attempt's included
      if (id === undefined) {
        violations.push(`agent_turn ${speechId}: generation without lk.generation_id`);
      } else if (index > lastHandoff && !String(id).startsWith(`${speechId}_`)) {
        violations.push(
          `agent_turn ${speechId}: generation ${String(id)} after the last handoff is not its own`,
        );
      }
    }
  }
