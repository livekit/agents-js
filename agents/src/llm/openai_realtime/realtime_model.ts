...
import { APIConnectionError, APIError, isAPIError } from '../../_exceptions.js';
import { AudioByteStream } from '../../audio.js';
import { log } from '../../log.js';
import type * as metrics from '../../metrics/index.js';
import * as stream from '../../stream/index.js';
import type { APIConnectOptions } from '../../types.js';
import { DEFAULT_API_CONNECT_OPTIONS } from '../../types.js';
import { Future, Queue, Task, cancelAndWait, delay, shortuuid } from '../../utils.js';
...
export class RealtimeModel extends llm.RealtimeModel {
  ...
  constructor(
    ...
    const modalities = (options.modalities ||
      DEFAULT_REALTIME_MODEL_OPTIONS.modalities) as api_proto.Modality[];
...
export function processBaseURL({
  ...
  if (!basePath.endsWith('/realtime')) {
    url.pathname = `${basePath}/${realtimePath}`;
  }
...
export class RealtimeSession extends llm.RealtimeSession {
  ...
  constructor(realtimeModel: RealtimeModel, options?: { turnDetectionDisabled?: boolean }) {
    ...
    if (options?.turnDetectionDisabled) {
  ...
  protected createSessionUpdateEvent(): api_proto.SessionUpdateEvent {
    ...
    if (opts.isAzure && opts.apiVersion) {
      ...
      const modalities: api_proto.Modality[] = opts.modalities.includes('audio')
        ? ['text', 'audio']
        : ['text'];
      return {
        type: 'session.update',
    ...
    const modality: api_proto.Modality = opts.modalities.includes('audio') ? 'audio' : 'text';
  ...
  private async runWs(wsConn: WebSocket): Promise<void> {
    ...
    const waitReconnectTask =
      this._options.maxSessionDuration === null
        ? undefined
    ...
    try {
      const result = await Promise.race([
        wsTask.result,
        sendTask.result,
        ...(waitReconnectTask ? [waitReconnectTask.result] : []),
      ]);
      ...
      await cancelAndWait(
        [wsTask, sendTask, ...(waitReconnectTask ? [waitReconnectTask] : [])],
        2000,
      );
      wsConn.close();
    }
  ...
  private handleResponseOutputItemAdded(event: api_proto.ResponseOutputItemAddedEvent): void {
    ...
    const modalitiesFut = new Future<api_proto.Modality[]>();
  ...
  private handleResponseContentPartAdded(event: api_proto.ResponseContentPartAddedEvent): void {
    ...
    if (!itemGeneration.modalities.done) {
      const modalityResult: api_proto.Modality[] = isTextType ? ['text'] : ['audio', 'text'];
  ...
  private handleResponseDoneButNotComplete(event: api_proto.ResponseDoneEvent): void {
    ...
    if (event.response.status === 'failed') {
      ...
      const recoverable = !this.isFatalError(errorBody);
  ...
  private handleError(event: api_proto.ErrorEvent): void {
    ...
    if (eventId) {
      ...
      if (future) {
        ...
        if (!this.isFatalError(event.error)) return;
      }
    }
    ...
    const recoverable = !this.isFatalError(event.error);
