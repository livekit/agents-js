...
export class ChatContext {
  ...
  /**
   * Remove items to only keep the newest `maxItems` elements. Logic mirrors the
   * behaviour in the Python implementation.
   */
  ...
  /**
   * Convert to provider-specific format. Currently just a stub so the public
   * API remains compatible with Python.
   */
  ...
  /**
   * Indicates whether the context is read-only – always `false` for now. We'll
   * port the read-only wrapper once a use-case in JS appears.
   */
