...
export class Worker {
  ...
  /** @throws MissingCredentialsError if URL, API key or API secret are missing */
  ...
  /** @throws WorkerError if worker failed to connect or already running */
  async run() {
    if (!this.#closed) {
  ...
  /** @throws WorkerError if worker did not drain in time */
