...
export interface AudioRecognitionOptions {
  ...
  /** Endpointing state used to select the end-of-turn delay. */
...
export class AudioRecognition {
  ...
  /** @internal */
  // Ref: python livekit-agents/livekit/agents/voice/audio_recognition.py - 193-219 lines
  ...
  async onStartOfSpeech(startedAt: number, speechDuration = 0, userSpeakingSpan?: Span) {
    ...
    return this.onStartOfOverlapSpeech(speechDuration, startedAt, userSpeakingSpan);
  }
  ...
  private runEOUDetection(chatCtx: ChatContext) {
    ...
    const bounceEOUTask =
      ...
      async (controller: AbortController) => {
        // Ref: python livekit-agents/livekit/agents/voice/audio_recognition.py - 949-973 lines
