...
import { BaseEndpointing, type BaseEndpointing as BaseEndpointingType } from './endpointing.js';
...
export interface AudioRecognitionOptions {
  ...
  endpointing?: BaseEndpointingType;
...
export class AudioRecognition {
  ...
  private endpointing: BaseEndpointingType;
  ...
  constructor(opts: AudioRecognitionOptions) {
    ...
    this.endpointing =
      opts.endpointing ?? new BaseEndpointing(opts.minEndpointingDelay, opts.maxEndpointingDelay);
  ...
  onEndOfSpeech(endedAt: number, userSpeakingSpan?: Span, interruption?: boolean) {
    if (this.speaking) {
  ...
  private runEOUDetection(chatCtx: ChatContext) {
    ...
    const bounceEOUTask =
      ...
      async (controller: AbortController) => {
        // Ref: python livekit-agents/livekit/agents/voice/audio_recognition.py - 928-959 lines
