...
import {
  ...
  functionCallToWire,
  type getAgentInfoResponseSchema,
  type getChatHistoryResponseSchema,
  type getRTCStatsResponseSchema,
  type getSessionStateResponseSchema,
  type getSessionUsageResponseSchema,
  msToS,
  sendMessageRequestSchema,
  type sendMessageResponseSchema,
  streamRequestSchema,
  type streamResponseSchema,
} from './wire_format.js';
...
function clientSerializeOptions(opts: {
...
// ===========================================================================
// ClientEventsHandler (deprecated JSON-based server-side handler)
// ===========================================================================

/**
 * @deprecated Use {@link SessionHost} with {@link RoomSessionTransport} instead.
 *
...
export class ClientEventsHandler {
  ...
  private async handleStreamRequest(
    ...
    try {
      ...
      this._logger.warn({ error: e }, 'failed to handle stream request');
    }
  }
  ...
  private async sendClientEvent(event: ClientEvent): Promise<void> {
    ...
    try {
      ...
      this._logger.warn({ error: e }, 'failed to stream event to clients');
    }
  }
  ...
  private onUserTextInput = (
    ...
    if (!participant) {
      this._logger.warn('participant not found, ignoring text input');
      return;
    }

    if (!this.textInputCb) {
      this._logger.error('text input callback is not set, ignoring text input');
      return;
    }
