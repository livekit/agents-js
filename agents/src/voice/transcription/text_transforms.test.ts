...
describe('textTransforms.filterMarkdown', () => {
  const emphasisCases = [
    ...
    ['Nested (**bold**) text.', 'Nested (bold) text.'],
    ['Quoted “**important**” word.', 'Quoted “important” word.'],
  ...
  for (const [text, expected] of emphasisCases) {
    for (const chunkSize of [1, 2, 3, 7, 50]) {
      it(`strips emphasis for ${JSON.stringify(text)} with chunk size ${chunkSize}`, async () => {
  ...
  for (const text of preserveCases) {
    for (const chunkSize of [1, 3, 7, 50]) {
      it(`preserves non-markdown ${JSON.stringify(text)} with chunk size ${chunkSize}`, async () => {
  ...
  for (const text of [
    ...
    it(`is independent of chunking for ${JSON.stringify(text)}`, async () => {
      const outputs = new Set(
        await Promise.all([1, 2, 3, 5, 7, 11, 50, 1000].map((size) => filtered(text, size))),
      );
      expect(outputs.size, `chunk-size dependent output: ${JSON.stringify([...outputs])}`).toBe(1);
    });
  }
});
