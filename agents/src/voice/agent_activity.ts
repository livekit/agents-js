...
export class AgentActivity implements RecognitionHooks {
  ...
  async updateTools(tools: readonly ToolContextEntry<any>[]): Promise<void> {
    ...
    // Run lifecycle calls in the order setup → swap toolCtx → close so a `setup()` failure
    // leaves the agent pointing at the OLD toolCtx. The removed toolsets stay tracked by
    // `agent.toolCtx.toolsets` and will be closed by the activity's normal teardown path —
    // no leak even if `setupToolsets` throws and propagates. `setupToolsets()` already rolls
    // back its own partial successes internally.
  ...
  private async setupToolsets(
    ...
    // rather than silently advertising tools whose backing resources never initialized.
    // If any setup fails, close the ones that already succeeded to avoid leaking their
    // backing resources — `closeToolsets()` won't run them after a throw because
    // `_toolsetsSetup` never flipped to true.
