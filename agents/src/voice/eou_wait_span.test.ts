...
describe.sequential('eou_wait span', () => {
  ...
  describe('full session', () => {
    ...
    it('keeps the turn open for recognition when the hook returns at once', async () => {
      // the activity adopts user_turn at end of turn and ends it after onUserTurnCompleted;
      // recognition stamps the transcript and closes eou_wait on its own continuation. With a
      // hook that returns immediately the task's `finally` is the nearest thing to a race:
      // recognition must still get its stamping in before the activity ends the span
      const agent = new HookAgent(async () => {});
      await runOneTurn(agent);
      ...
      expect(wait.parentSpanContext?.spanId).toBe(userTurn.spanContext().spanId);
      expect(wait.attributes[traceTypes.ATTR_EOU_OUTCOME]).toBe('committed');
      // attributes set after a span ends are dropped: these prove recognition stamped first
      ...
      expect(userTurn.attributes[traceTypes.ATTR_END_OF_TURN_DELAY]).toBeTypeOf('number');
      // and the turn outlives both its wait and the hook (spans anchor their own clocks, so
      // allow the sub-millisecond disagreement the trace checker allows)
      const hook = only(exporter, 'on_user_turn_completed');
      expect(hook.parentSpanContext?.spanId).toBe(userTurn.spanContext().spanId);
