...
describe.sequential('coverage spans', () => {
  ...
  it('a committed user turn interrupts the queued replies for the same reason', async () => {
    ...
    const session = new AgentSession({
      ...
      stt,
      llm,
      // AEC warmup holds off barge-ins on audio activity (VAD, or a transcript arriving while
      // the agent speaks) without making the speech uninterruptible: the committed turn is
      // what interrupts, the case this test is about
      aecWarmupDuration: 30_000,
      turnHandling: {
        turnDetection: 'vad',
        endpointing: { minDelay: 100, maxDelay: 100 },
    ...
    session.input.audio = audioInput;
    session.output.audio = new PacedOutput();
    // 120 frames at 20 ms every 25 ms: ~3 s of playout, still going when the turn commits
    await session.start({ agent: new PacedAgent(120, 25) });
    let queued: SpeechHandle | undefined;
    let interrupted: ReadableSpan[] = [];
    try {
      audioInput.push(20);
      await delay(20);
      vad.startOfSpeech();
      ...
      vad.endOfSpeech();
      await waitFor(() => session.agentState === 'speaking', 10_000, 'the story to start playing');
      // say with audio: the paced agent has no TTS
      queued = session.say('And one more thing.', {
      ...
      await waitFor(
        () =>
          spansNamed(exporter, 'agent_turn').some(
            (span) =>
              span.attributes[traceTypes.ATTR_SPEECH_INTERRUPTED] === true &&
              traceTypes.ATTR_INTERRUPTION_SOURCE in span.attributes,
          ) && queued!.interrupted,
        10_000,
        'the user turn to interrupt the story and the queued say',
      );
      // before close(), which interrupts whatever is still playing programmatically
      interrupted = spansNamed(exporter, 'agent_turn').filter(
        (span) => span.attributes[traceTypes.ATTR_SPEECH_INTERRUPTED] === true,
      );
    } finally {
      await session.close();
    ...
    expect(queued!._interruptSource).toBe('user_turn');
    expect(interrupted.length).toBeGreaterThanOrEqual(1);
    for (const turn of interrupted) {
      expect(turn.attributes[traceTypes.ATTR_INTERRUPTION_SOURCE]).toBe('user_turn');
    }
  }, 15_000);
  ...
  it('a cancelled preemptive attempt names why it was dropped', () => {
    // superseded by more of the user's turn, or dropped by a barge-in: the cause travels with
    // the cancel like it does with interrupt, and the first one stands
    const superseded = SpeechHandle.create()._cancel('user_turn');
    expect(superseded._interruptSource).toBe('user_turn');
    const bargedIn = SpeechHandle.create()._cancel('audio_activity');
    bargedIn.interrupt(false, 'programmatic');
    expect(bargedIn._interruptSource).toBe('audio_activity');
    // a cancel with no cause (teardown, pause) stays programmatic in the trace
    expect(SpeechHandle.create()._cancel()._interruptSource).toBeUndefined();
  });

  // -- user_turn names the STT --

  class NamedSTT extends FakeSTT {
  ...
  it('names the STT model and provider on user_turn, as python does', async () => {
    ...
    try {
      ...
      await waitFor(
        () =>
          spansNamed(exporter, 'user_turn').some(
            (span) => traceTypes.ATTR_USER_TRANSCRIPT in span.attributes,
          ),
        10_000,
        'the user turn to end',
      );
    } finally {
      await session.close();
    }
  ...
  class PartialLLMStream extends LLMStream {
    protected async run(): Promise<void> {
      this.queue.put({ id: 'partial', delta: { role: 'assistant', content: 'Hel' } });
      throw new APIConnectionError({ message: 'primary dropped mid-response' });
    }
  }

  class PartialLLM extends LLM {
    label(): string {
      return 'partial-llm';
    }
    ...
    chat(opts: {
      ...
      return new PartialLLMStream(this, {
        chatCtx: opts.chatCtx,
        toolCtx: opts.toolCtx,
        connOptions: opts.connOptions ?? DEFAULT_API_CONNECT_OPTIONS,
      });
  ...
  it('names the instance whose partial response the caller received before it failed', async () => {
    // with retryOnChunkSent off the failure is rethrown after chunks reached the caller: the
    // spans still say who produced them, as the TTS fallback does for audio already heard
    const primary = new PartialLLM();
    ...
    let text = '';
    // the stream ends and the failure is reported on the adapter's error event
    const failure = new Promise<Error>((resolve) => adapter.on('error', (e) => resolve(e.error)));
    ...
    await expect(failure).resolves.toBeInstanceOf(APIConnectionError);

    const run = spansNamed(exporter, 'llm_request_run').find(
      (span) => traceTypes.ATTR_FALLBACK_LABEL in span.attributes,
    );
    expect(run?.attributes[traceTypes.ATTR_FALLBACK_LABEL]).toBe(primary.label());
    // the adapter's request span (the instance's own llm_request nests under the run)
    const request = spansNamed(exporter, 'llm_request').find(
      (span) => span.spanContext().spanId === run?.parentSpanContext?.spanId,
    )!;
    expect(request.attributes[traceTypes.ATTR_GEN_AI_RESPONSE_MODEL]).toBe(primary.model);
    expect(request.attributes[traceTypes.ATTR_GEN_AI_PROVIDER_NAME]).toBe('partial');
