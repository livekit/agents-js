...
> extends (EventEmitter as new () => TypedEmitter<AgentSessionCallbacks>) {
  ...
  private async closeImplInner(
    ...
    // teardown as one bar under agent_session; drain and on_exit nest inside it. The rest of
    // the close (the close event, room io) stays under agent_session.
    ...
    try {
      await otelContext.with(closeContext, () =>
        this.teardownActivity(wasStarted, reason, drain, closeContext),
      );
    } finally {
      closeSpan.end();
    }
