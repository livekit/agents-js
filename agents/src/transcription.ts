...
import { SentenceStream, SentenceTokenizer } from "./tokenize/tokenizer.js"
...
export class TextAudioSynchronizer extends (EventEmitter as new () => TypedEmitter<SyncCallbacks>) {
  ...
  async #syncSentence(segIndex: number, segStartTime: number, textData: TextData, audioData: AudioData, sentence: string) {
  ...
  async #sleepIfNotClosed(delay: number) {
    await Promise.race([this.#closeFut.await, new Promise(resolve => setTimeout(resolve, delay))])
  }
