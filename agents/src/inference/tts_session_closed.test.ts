...
/** How many sentences the reply keeps pushing while the gateway drops the session. */
const LATE_PUSHES = 30;
...
describe('inference TTS dropped gateway session', () => {
  ...
  it('keeps the attempt retryable when text arrives while the drop is being handled', async () => {
    const tts = createTTS(gateway.baseURL);
    ...
    // A reply is pushed as it is generated, so text keeps arriving while its audio plays
    // out — and therefore while the gateway drops the session. Pushing from the audio
    // consumer puts the push in the same turn of the loop that handles `session.closed`,
    // which is the ordering the production trace hit.
    let pushes = 0;
    const consumed = (async () => {
      for await (const event of stream) {
        if (typeof event === 'symbol') continue;
        if (pushes < LATE_PUSHES) {
          pushes++;
          stream.pushText(`Sentence number ${pushes} of the reply. `);
        }
      }
    })();
    ...
    // Cleanup closes the sentence tokenizer, so a text chunk that arrives during the drop
    // would be pushed into a closed stream. That throws a plain `Error`, which is not an
    // `APIError` and so is reported as unrecoverable and never retried — which would defeat
    // the retry this whole path exists for.
    expect(
      errors.filter((error) => !error.recoverable).map((error) => error.error.message),
    ).toEqual([]);
    expect(gateway.connections.length).toBeGreaterThan(1);
