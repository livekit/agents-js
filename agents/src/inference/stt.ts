...
/** Confidence and sensitivity knobs are probabilities: 0..1. */
...
/** Durations are never negative, whatever their unit. */
...
function checkedEotThreshold(value: number): number {
  console.assert(value >= 0.5);
  console.assert(value <= 0.9);
  return value;
}
...
/** Speaker counts are positive integers. */
function checkedSpeakerCount(value: number): number {
...
/**
 * Caller requirements for the ranged provider option fields.
 *
 * Numeric fields are spelled out per provider rather than reusing the option
 * interfaces: freerange stops analyzing a function that reads a union-typed
 * field (`keyterm?: string | string[]`), and it does not analyze mapped types.
 */
/** The ranged numeric fields across every provider's STT option interface. */
type RangedSTTModelOptions = {
  // Cartesia
  turn_start_threshold?: number;
  turn_eager_end_threshold?: number;
  turn_end_threshold?: number;
...
function checkDeepgramFluxOptionRanges(modelOptions: RangedSTTModelOptions): void {
  ...
  if (modelOptions.eot_threshold !== undefined) checkedEotThreshold(modelOptions.eot_threshold);
...
function checkSpeechmaticsOptionRanges(modelOptions: RangedSTTModelOptions): void {
  ...
  if (modelOptions.max_speakers !== undefined) checkedSpeakerCount(modelOptions.max_speakers);
 ...
 */
function checkSTTModelOptionRanges(
