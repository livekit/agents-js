...
describe('Twilio connector CallToken forwarding', () => {
  ...
  it('cancels an unanswered forwarded call without reusing the token in the cancellation', async () => {
    ...
    expectDialFailure(ctx, 'recipient did not answer');
  });
...
describe('Twilio caller-ID fallback', () => {
  ...
  it('does not retry an ambiguous network timeout', async () => {
    ...
    await ctx.enter();
    expect(ctx.fetch).toHaveBeenCalledOnce();
    expectDialFailure(ctx, 'Twilio call creation request failed');
  });
...
describe('Twilio provider timeout and redaction', () => {
  it.each([
    [0, '5'],
    [1501, '5'],
    [30_000, '30'],
    ...
    'sends the provider timeout for %s ms on initial and fallback calls',
    async (ringingTimeout, expected) => {
      const ctx = setupTransfer({ callToken: CALL_TOKEN, ringingTimeout });
      ctx.fetch.mockImplementationOnce(responds('{"code":21210}', { status: 400 }));
      await ctx.enter();
      expect(formOf(ctx.fetch, 0).get('Timeout')).toBe(expected);
      expect(formOf(ctx.fetch, 1).get('Timeout')).toBe(expected);
    },
  );

  it.each(['not json', '{"message":"secret"}', 'null'])(
    'does not retry or expose malformed successful response %s',
    async (body) => {
      const ctx = setupTransfer({ callToken: CALL_TOKEN });
      ctx.fetch.mockImplementationOnce(responds(body));
      await ctx.enter();
      expect(ctx.fetch).toHaveBeenCalledOnce();
      expectDialFailure(ctx, expect.stringContaining('Twilio call creation returned'));
      expect(JSON.stringify(ctx.logError.mock.calls)).not.toContain(body);
    },
  );

  it('does not log sensitive transport errors or retry them', async () => {
    const ctx = setupTransfer({ callToken: CALL_TOKEN });
    ctx.fetch.mockRejectedValueOnce(new Error(`request failed: ${CALL_TOKEN} ${CALLER_NUMBER}`));
    await ctx.enter();
    expect(ctx.fetch).toHaveBeenCalledOnce();
    expectDialFailure(ctx, 'Twilio call creation request failed');
    expect(JSON.stringify(ctx.logError.mock.calls)).not.toContain(CALL_TOKEN);
    expect(JSON.stringify(ctx.logError.mock.calls)).not.toContain(CALLER_NUMBER);
  });
});
