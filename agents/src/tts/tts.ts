  ...
  implements AsyncIterableIterator<SynthesizedAudio | typeof SynthesizeStream.END_OF_STREAM>
{
  ...
  private _mainTaskImpl = async (span: Span) => {
    ...
    span.setAttributes({
      ...
      // Cost is owned by the outer `tts_node` generation. Marking
      // `tts_request` as `span` prevents tracing backends from billing the
      // FallbackAdapter wrapper + inner provider layers as a separate cost.
