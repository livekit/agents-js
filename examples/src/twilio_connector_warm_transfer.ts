...
/**
 * Warm transfer with the recipient dialed through the Twilio connector.
 *
 ...
 * connector session and the Twilio REST API places the recipient call with
...
const TWILIO_FROM_NUMBER = process.env.TWILIO_FROM_NUMBER; // your Twilio number, shown to the recipient
const TRANSFER_PHONE_NUMBER = process.env.LIVEKIT_TRANSFER_PHONE_NUMBER; // "+12003004000"
    ...
    super({
      ...
      tools: [
        llm.tool({
          ...
          description: `Called when the user asks to speak to a human agent. This will put the user on hold while the recipient is connected.
...
----
- User: Can I speak to your recipient?
...
----`,
          execute: async (_, { ctx }) => {
            ...
            try {
              if (!TRANSFER_PHONE_NUMBER || !TWILIO_FROM_NUMBER) {
                throw new Error('LIVEKIT_TRANSFER_PHONE_NUMBER and TWILIO_FROM_NUMBER must be set');
              }

              const result = await new workflows.TwilioConnectorWarmTransferTask({
                phoneNumber: TRANSFER_PHONE_NUMBER,
                twilioFromNumber: TWILIO_FROM_NUMBER,
                twilioAccountSid: TWILIO_ACCOUNT_SID,
                twilioAuthToken: TWILIO_AUTH_TOKEN,
                // To preserve caller ID, load originalCallerNumber and twilioCallToken
                // from server-side state for the same validated inbound webhook.
                chatCtx: ctx.session.history,
                // Give up if the recipient doesn't pick up within 25s with
