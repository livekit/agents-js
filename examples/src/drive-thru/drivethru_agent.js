...
import { z } from 'zod';
import { COMMON_INSTRUCTIONS, FakeDB, findItemsById, menuInstructions, } from './database.js';
import { OrderState, createOrderedCombo, createOrderedHappy, createOrderedRegular, } from './order.js';
class DriveThruAgent extends voice.Agent {
    constructor(userdata) {
        const instructions = COMMON_INSTRUCTIONS +
            '\n\n' +
        ...
        super({
            instructions,
            tools: {
                orderComboMeal: DriveThruAgent.buildComboOrderTool(userdata.comboItems, userdata.drinkItems, userdata.sauceItems),
                orderHappyMeal: DriveThruAgent.buildHappyOrderTool(userdata.happyItems, userdata.drinkItems, userdata.sauceItems),
                orderRegularItem: DriveThruAgent.buildRegularOrderTool(userdata.regularItems, userdata.drinkItems, userdata.sauceItems),
...
If the \`orderId\`s are unknown, call \`listOrderItems\` first to retrieve them.`,
                    parameters: z.object({
                        orderIds: z
                            .array(z.string())
                            .describe('A list of internal `orderId`s of the items to remove. Use `listOrderItems` to look it up if needed.'),
                    }),
                    execute: async ({ orderIds }, { ctx }) => {
                        const notFound = orderIds.filter((oid) => !ctx.userData.order.items[oid]);
                        if (notFound.length > 0) {
                            throw new llm.ToolError(`error: no item(s) found with order_id(s): ${notFound.join(', ')}`);
                        }
                        const removedItems = await Promise.all(orderIds.map((oid) => ctx.userData.order.remove(oid)));
...
- User asks about current order details (e.g., "What's in my order so far?").`,
    ...
    static buildComboOrderTool(comboItems, drinkItems, sauceItems) {
...
If the user says just "a large meal," assume both drink and fries are that size.`,
            parameters: z.object({
                ...
                sauceId: z
                    .enum(availableSauceIds)
                    .nullable()
                    .describe('The ID of the sauce the user requested.'),
            }),
            execute: async ({ mealId, drinkId, drinkSize, friesSize, sauceId }, { ctx }) => {
                ...
                if (actualDrinkSize === undefined && availableSizes.length > 1) {
                    throw new llm.ToolError(`error: ${drinkId} comes with multiple sizes: ${availableSizes.join(', ')}. Please clarify which size should be selected.`);
                }
                if (actualDrinkSize !== undefined && !availableSizes.length) {
                    throw new llm.ToolError(`error: size should not be specified for item ${drinkId} as it does not support sizing options.`);
                }
    ...
    static buildHappyOrderTool(happyItems, drinkItems, sauceItems) {
...
Assume Small as default only if the user says "Happy Meal" and gives no size preference, but always ask for clarification if unsure.`,
            ...
            execute: async ({ mealId, drinkId, drinkSize, sauceId }, { ctx }) => {
    ...
    static buildRegularOrderTool(regularItems, drinkItems, sauceItems) {
...
- "Can I get a McFlurry Oreo?"`,
            ...
            execute: async ({ itemId, size }, { ctx }) => {
                ...
                if (actualSize === undefined && availableSizes.length > 1) {
                    throw new llm.ToolError(`${itemId} comes with multiple sizes: ${availableSizes.join(', ')}. Please clarify which size should be selected.`);
                }
                ...
                if (actualSize && availableSizes.length && !availableSizes.includes(actualSize)) {
                    throw new llm.ToolError(`error: unknown size ${actualSize} for ${itemId}. Available sizes: ${availableSizes.join(', ')}.`);
                }
...
//# sourceMappingURL=drivethru_agent.js.map