...
// Helper functions
export function findItemsById(items, itemId, size) {
    return items.filter((item) => item.id === itemId && (size === undefined || item.size === size));
}
export function menuInstructions(category, items) {
    switch (category) {
        case 'drink':
...
function drinkMenuInstructions(items) {
...
function comboMenuInstructions(items) {
    const menuLines = [];
    for (const item of items) {
...
function happyMenuInstructions(items) {
    const menuLines = [];
    for (const item of items) {
...
function sauceMenuInstructions(items) {
    const menuLines = [];
    for (const item of items) {
        let line = `  - ${item.name}: ${item.calories} Cal, $${item.price.toFixed(2)} (id:${item.id})`;
        if (!item.available) {
...
function regularMenuInstructions(items) {
    const { sized, leftovers } = mapBySizes(items);
    const menuLines = [];
    for (const [_itemId, sizeMap] of Object.entries(sized)) {
        const firstItem = Object.values(sizeMap)[0];
