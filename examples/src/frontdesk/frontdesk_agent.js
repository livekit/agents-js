...
import { z } from 'zod';
import { CalComCalendar, FakeCalendar, SlotUnavailableError, getUniqueHash, } from './calendar_api.js';
export class FrontDeskAgent extends voice.Agent {
    ...
    constructor(options) {
        ...
        const instructions = `You are Front-Desk, a helpful and efficient voice assistant. ` +
            ...
            tools: {
                scheduleAppointment: llm.tool({
                    ...
                    parameters: z.object({
                        slotId: z
                            .string()
                            .describe('The identifier for the selected time slot (as shown in the list of available slots).'),
                    }),
                    execute: async ({ slotId }, { ctx }) => {
                        ...
                        console.warn('Note: Email collection workflow not implemented in TypeScript version yet. Using placeholder email.');
                        try {
...
export default defineAgent({
    ...
    entry: async (ctx) => {
        ...
        if (calApiKey) {
            ...
            console.warn('CAL_API_KEY is not set. Falling back to FakeCalendar; set CAL_API_KEY to enable Cal.com integration.');
...
//# sourceMappingURL=frontdesk_agent.js.map