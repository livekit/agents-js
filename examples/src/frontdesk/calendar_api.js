...
export function createAvailableSlot(startTime, durationMin) {
...
export function getUniqueHash(slot) {
...
export class FakeCalendar {
    ...
    async initialize() { }
    async scheduleAppointment(options) {
        // Fake it by just removing it from our slots list
        this._slots = this._slots.filter((slot) => slot.startTime.getTime() !== options.startTime.getTime());
    }
    async listAvailableSlots(options) {
        return this._slots.filter((slot) => slot.startTime >= options.startTime && slot.startTime < options.endTime);
    }
}
// --- cal.com impl ---
...
export class CalComCalendar {
    ...
    constructor(options) {
        ...
        this._logger = {
            info: (message) => console.info(`[cal.com] ${message}`),
        };
    }
    async initialize() {
        ...
        const lkEventType = data.find((event) => event.slug === _CAL_COM_EVENT_TYPE) || null;
        if (lkEventType) {
            ...
            if (!createResponse.ok) {
                throw new Error(`Failed to create event type: ${createResponse.status} ${createResponse.statusText}`);
            }
...
//# sourceMappingURL=calendar_api.js.map