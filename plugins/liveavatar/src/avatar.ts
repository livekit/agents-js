...
export class AvatarSession extends voice.AvatarSession {
  ...
  async start(
    ...
    this.mainTaskPromise = this.mainTask().catch((e) => {
      this.#logger.warn({ error: e }, 'LiveAvatar main task failed');
    });
  }
  ...
  private async mainTask(): Promise<void> {
    ...
    try {
        ...
        try {
          while (true) {
            ...
            try {
              ...
              this.#logger.warn({ error: e }, 'failed to send LiveAvatar event');
              break;
            }
          }
        } finally {
      ...
      this.#logger.warn({ error: e }, 'LiveAvatar main task error');
    } finally {
      this.closing = true;
      closingResolver.resolve();
      ...
      try {
        ...
        this.#logger.warn({ error: e }, 'Failed to stop LiveAvatar session');
      }
