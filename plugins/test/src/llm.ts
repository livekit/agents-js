...
export const llm = async (llm: llmlib.LLM, skipOptionalArgs: boolean) => {
  ...
  describe('LLM', async () => {
    ...
    describe('toolset', async () => {
      // Tools registered through a Toolset must reach the underlying LLM exactly like
      // top-level tools: the plugin must flatten the toolset's contents before advertising
      // tools to the provider, and the model must be able to call them.
      const buildToolsetContext = () => {
        ...
        // Mix a direct top-level tool with the Toolset so we also confirm that direct tools
