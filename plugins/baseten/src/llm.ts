...
export class LLM extends OpenAILLM {
    constructor(opts: BasetenLLMOptions) {
        ...
        if (!apiKey) {
            throw new Error('Baseten API key is required. Set BASETEN_API_KEY environment variable or pass apiKey in options.')
        }

        if (!opts.model) {
            throw new Error('Model is required. Please specify a model name (e.g., "openai/gpt-4o-mini").')
        }
