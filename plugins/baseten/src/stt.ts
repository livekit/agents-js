import { AudioByteStream, log, stt, type AudioBuffer } from '@livekit/agents'
...
export class STT extends stt.STT {
    ...
    constructor(opts: Partial<BasetenSttOptions> = {}) {
        ...
        if (!apiKey) {
            throw new Error('Baseten API key is required, either pass it as `apiKey` or set $BASETEN_API_KEY')
        }
        if (!modelId) {
        ...
        while (!this.input.closed && !this.closed) {
            ...
            try {
                ...
                if (!this.closed && !this.input.closed) {
                    ...
                    this.#logger.warn(`failed to connect to Baseten, retrying in ${delay} seconds: ${e} (${retries}/${maxRetry})`)
