...
```bash
pnpm add @livekit/agents-plugin-meta
```
...
You can also pass the key directly with `apiKey`. The resolution order is `apiKey`, `MODEL_API_KEY`, then `META_API_KEY`.
...
The plugin supports streaming recognition with server-side endpointing, cumulative interim transcripts, and mono PCM16 audio at 24 kHz. Connected streams must keep sending real-time PCM, including silence. `keywords` and `languageBias` are static hints applied during the initial handshake and cannot be changed on an active stream. Omit `languageBias` for automatic language detection.

Supported language names are: Arabic, Bengali, Dutch, English, French, German, Hebrew, Hindi, Indonesian, Italian, Japanese, Kannada, Korean, Malay, Mandarin Chinese, Marathi, Polish, Portuguese, Spanish, Tagalog, Tamil, Telugu, Thai, Turkish, and Vietnamese. The per-stream `language` option also accepts corresponding language codes and locales, such as `en-US`, `pt-BR`, and `zh-CN`, and maps them to the documented names.
