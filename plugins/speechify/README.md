...
Synthesis uses the Speechify `/v1/audio/stream/with-timestamps` endpoint (SSE)
via the official `@speechify/api` SDK, which streams raw PCM (24 kHz mono)
together with word-level speech marks as the audio is generated. `stream()`
chunks input into sentences and streams one request per sentence, emitting audio
and aligned word timestamps as they arrive. Every request is attributed to this
