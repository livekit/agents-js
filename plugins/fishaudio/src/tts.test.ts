...
async function synthesizeTurn(fishAudio: TTS, text: string): Promise<tts.SynthesizedAudio[]> {
  const stream = fishAudio.stream();
...
describe('FishAudio streaming', () => {
  ...
  it('retries on a fresh websocket after a connection failure', async () => {
    const { wss, baseURL } = await startWebSocketServer();
    let connectionCount = 0;
    ...
    const fishAudio = new TTS({ apiKey: 'test-key', baseURL });
    try {
      expect(await synthesizeTurn(fishAudio, 'retry this turn.')).not.toHaveLength(0);
