...
export class SpeechStream extends stt.SpeechStream {
  ...
  protected async run(): Promise<void> {
    ...
    try {
      ...
      const timeoutMs = this.#connOptions?.timeoutMs ?? 15000;
      let connTimeoutId: NodeJS.Timeout | undefined;
      const connTimeoutPromise = new Promise<any>((_, reject) => {
        connTimeoutId = setTimeout(
          () =>
            reject(
              new APIConnectionError({
                message: `Mistral STT: connection timed out after ${timeoutMs}ms`,
      ...
      connection = await Promise.race([
        client.connect(this.#stt.options.liveModel, {
          audioFormat: this.#audioFormat,
          // @ts-ignore Mistral STT stream API accepts 'language' despite SDK typings omitting it
          language: this.#stt.options.language,
        }),
        connTimeoutPromise,
      ]);
        ...
        if (event.type === 'transcription.language') {
          ...
          if ('words' in typedEvent && Array.isArray(typedEvent.words)) {
            parsedWords = typedEvent.words.map((w: any) =>
              createTimedString({
                text: w.word || w.text || '',
                startTime: (w.start || 0) + this.startTimeOffset,
                endTime: (w.end || 0) + this.startTimeOffset,
                confidence: w.confidence ?? 1.0,
              }),
            );
          }
            ...
            alternatives: [
              {
                ...
                language: normalizeLanguage(currentLanguage),
                startTime:
                  typedEvent.start !== undefined
                    ? typedEvent.start + this.startTimeOffset
                    : parsedWords && parsedWords.length > 0
                      ? parsedWords[0].startTime
                      : this.startTimeOffset,
                endTime:
                  typedEvent.end !== undefined
                    ? typedEvent.end + this.startTimeOffset
                    : parsedWords && parsedWords.length > 0
      ...
      if (!stopRequested && !doneReceived) {
        throw new APIConnectionError({
          message: 'Mistral STT: WebSocket closed unexpectedly without transcription.done',
          options: { retryable: true },
        });
      }
    } catch (error: unknown) {
