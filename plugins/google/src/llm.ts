...
export class LLMStream extends llm.LLMStream {
  ...
  protected async run(): Promise<void> {
    ...
    try {
      ...
      // Gemini rejects cachedContent requests that also send these fields. They must be
      // baked into the CachedContent resource when the cache is created.
      const usingCache = this.#extraKwargs.cachedContent !== undefined;
      const config: GenerateContentConfig = {
        ...this.#extraKwargs,
        systemInstruction: usingCache ? undefined : systemInstruction,
        httpOptions: this.#extraKwargs.httpOptions ?? {
          timeout: Math.floor(this.connOptions.timeoutMs),
        },
        tools: usingCache ? undefined : tools,
      };
      if (usingCache) {
